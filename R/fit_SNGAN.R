...
fit_SNGAN <- function(generator, discriminator,n.discriminator=5,batch=25,
  ...
  for (i in 1:epoch) {
    ...
    for (j in 1:n.discriminator) {
      ...
      d_loss_real <- discriminator%>%keras::train_on_batch(real,real_res)
      ...
      d_loss_fake <- discriminator%>%keras::train_on_batch(fake,fake_res)
    ...
    noise <- keras::k_random_normal(c(batch,l),mean = 0,stddev = 1)
    real_res <- array(runif(batch, -1 ,  -1),dim =c(batch,1))
    gloss[i]  <- GAN%>%keras::train_on_batch(noise,real_res)
